...
- drive LOD and frustum culling from a separate `camera`
- keep gameplay terrain resident with `getResidencyAnchors`
...
By default the runner uses the active R3F canvas camera for both rendering and quadtree updates. Pass `camera` when you want a separate camera to drive LOD refinement and frustum culling while the canvas camera stays free for viewing.
...
You can pass `camera` to `useTerrain()` or directly to `<Terrain camera={...} />`. When both are provided, the `<Terrain />` prop wins.

When `getCameraOrigin` is omitted, the tracked origin defaults to the active camera's position (custom or canvas). Provide `getCameraOrigin` when the quadtree should follow a player or probe instead of the camera.

## Keeping Gameplay Terrain Resident

Render culling should not decide whether terrain exists for physics, character controllers, or queries. Use `getResidencyAnchors` to publish world-space support regions that should stay resident even when they are outside the render frustum. See [Residency Anchors](/docs/core/residency-anchors) for the full API model.
