...
- GPU render-side frustum culling by default, with optional previous-frame Hi-Z occlusion culling.
...
Render visibility optimizations stay renderer-facing: the React runtime captures an internal depth pyramid for Hi-Z occlusion, while compute, query, and raycast continue to operate on the full quadtree leaf set.
