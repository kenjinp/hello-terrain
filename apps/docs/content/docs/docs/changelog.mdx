...
Per-frame terrain inputs are split into focused params with equality gating and run-owned concurrency. React options are regrouped into `culling`, `residency`, `lod`, and `pipeline`.
...
Vanilla and React consumers follow the same loop:
