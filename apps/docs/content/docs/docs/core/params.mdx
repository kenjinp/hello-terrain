...
| `quadtreeUpdate` | `UpdateParams` | `{ cameraOrigin: { x: 0, y: 0, z: 0 }, mode: "distance", distanceFactor: 1.5 }` | The camera-relative quadtree refinement settings, including the tracked origin, optional view-projection matrix for frustum culling, residency anchors for physics/query support, and update mode. |
...
- `camera` drives `quadtreeUpdate` view-projection through `writeUpdateParamsFromCamera`. Defaults to the R3F canvas camera.
- `getCameraOrigin` and `cameraHysteresis` influence how the runner updates the core `quadtreeUpdate` param over time.
- `getResidencyAnchors` maps to `quadtreeUpdate.residency.anchors`, keeping terrain data resident around gameplay systems even when render culling hides those tiles.
