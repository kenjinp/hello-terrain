...
| `stitchSeams` | `boolean` | `true` | Stitch T-junction seams across 2:1 LOD boundaries (flat and cube-sphere): odd boundary vertices on edges facing a coarser neighbor are snapped (position and normal) onto the chord between their even edge-neighbors, removing cracks and shading discontinuities. Skirts stay enabled as a safety net. Watertight when `innerTileSegments` is even. |
...
| `topology` | `Topology` | `null` | `null` | The terrain topology that defines tile layout, bounds, and GPU projection. Leave it unset for the default flat topology, or pass `createInfiniteFlatTopology` / `createCubeSphereTopology`. See [Topology Types](/docs/core/topology). |
