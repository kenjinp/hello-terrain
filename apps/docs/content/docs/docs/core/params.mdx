...
| `terrainReadbackEnabled` | `boolean` | `true` | Whether `terrainReadbackTask` schedules the GPU→CPU elevation readback that powers `TerrainQuery`, `TerrainRaycast`, and surface-relative LOD. Set to `false` when nothing on the CPU needs terrain data. See [Controlling readback](/docs/core/query/how-it-works#controlling-readback). React exposes this as the `terrainReadback` option. |
| `terrainReadbackIntervalMs` | `number` | `0` | Minimum milliseconds between scheduled readbacks. `0` reads back whenever the previous readback finished and the quadtree changed; larger values throttle the ~`activeLeafCount × (innerTileSegments + 3)² × 4` byte copy at the cost of a staler CPU snapshot. |
...
- `terrainReadback` maps to `terrainReadbackEnabled`; `terrainReadbackIntervalMs` maps to the param of the same name.
