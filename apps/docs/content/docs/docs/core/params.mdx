...
| `radius` | `number` | `1000` | The sphere radius used by the cube-sphere projection. Ignored by flat surfaces. See [Cube-Sphere Planet](/docs/examples/cube-sphere). |
...
| `surface` | `Surface` | `null` | `null` | The terrain surface model that defines topology, bounds, and GPU projection. Leave it unset for the default flat surface, or pass `createInfiniteFlatSurface` / `createCubeSphereSurface`. |
