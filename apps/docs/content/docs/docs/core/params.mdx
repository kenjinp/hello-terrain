...
Terrain configuration in Hello Terrain is built on top of the `param()` / `ParamRef` system from `@hello-terrain/work`.
...
| `quadtreeUpdate` | `UpdateParams` | `{ cameraOrigin: { x: 0, y: 0, z: 0 }, mode: "distance", distanceFactor: 1.5 }` | The camera-relative quadtree refinement settings, including the tracked origin and LOD criteria. In `mode: "distance"` a tile splits when the camera is within `distanceFactor × r` of its bounding sphere (`r` is the surface-relative bounds radius); `distanceFactor` falls back to `1.5` when omitted. `mode: "screen"` requires `projectionFactor` and `targetPixels` instead. |
