...
type ComputeStageCallback = (
  ...
  uv: Node,                 // vec2 — tile-local coordinates in [0, 1)
...
const erosionStageTask = task((get, work) => {
  ...
  return work((): ComputePipeline => [
    ...
    (nodeIndex, globalVertexIndex, uv) => {
      // Read/modify the elevation field written by the previous stage.
      const height = elevationField.node.element(globalVertexIndex);
      height.assign(height.mul(0.9));
    },
  ]);
}).displayName("erosionStageTask");

const { compile, execute } = createComputePipelineTasks(erosionStageTask);

const graph = terrainGraph();
graph.add(execute); // `compile` is registered automatically as an upstream dependency
```
...
- When a pipeline has more than one stage, all stages but the last run first, the per-tile elevation bounds are reduced mid-pipeline, and then the final stage runs. Keep the terrain-field pack stage (or any stage that reads final elevation bounds) last.
