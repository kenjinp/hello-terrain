...
flowchart TD
    ...
    ElevationBuffer --> ElevReadback["getArrayBufferAsync()"]
    BoundsBuffer --> BoundsReadback["getArrayBufferAsync()"]
...
4. **Readback** is triggered once per frame (if no readback is already in flight). It clones the current `SpatialIndex` into a back buffer, then fires two parallel `getArrayBufferAsync` calls — one for the elevation buffer and one for the small per-tile bounds buffer. Both are asynchronous and resolve on a later frame.
...
- `terrainQueryTask` depends on `tileBoundsReductionTask` (which transitively depends on `executeComputeTask`) and `quadtreeConfigTask`. It triggers readback of both the elevation buffer and the bounds buffer each frame.

- `terrainRaycastTask` depends on `terrainQueryTask`. It reads the global elevation range from the query to set its AABB, falling back to a conservative estimate before the first readback completes.
...
| `getTile` / `getTileBounds` | < 0.01 ms | Hash lookup only |
| `getGlobalElevationRange` | < 0.01 ms | Precomputed during readback swap |
| GPU bounds reduction | < 0.1 ms | Parallel shared-memory reduction per tile |
