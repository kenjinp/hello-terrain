...
Its methods take a world position (projected onto the surface) — a
`THREE.Vector3` or any plain `{ x, y, z }`; results are always `THREE.Vector3`:
...
- `...ByDirection(direction)` — `direction` is a vector (`THREE.Vector3` or plain `{ x, y, z }`) from the planet center (normalized internally). This is the canonical form.
- `...ByPosition(position)` — any world-space position (`THREE.Vector3` or plain `{ x, y, z }`); it is projected onto its direction from the center.
