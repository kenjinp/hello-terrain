...
- Readback can be throttled or disabled with the `terrainReadbackEnabled` / `terrainReadbackIntervalMs` params. When disabled, queries stay `{ valid: false }` forever. See [Controlling readback](/docs/core/query/how-it-works#controlling-readback).
