...
Returns world-space displacement min/max when data is available; `false` when the tile has no readback yet (datum-only bounds for that frame). This mirrors the one-frame latency of `elevationAtCameraXZ`.
