...
`viewProjectionMatrix` is the world-to-clip matrix used for frustum culling. `tileElevationRange` returns world-space displacement min/max when data is available; `false` when the tile has no readback yet (datum-only bounds for that frame, a one-frame latency until readback data arrives).
