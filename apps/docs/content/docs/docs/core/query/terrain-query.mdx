...
The `TerrainQuery` above describes a flat heightfield keyed on world XZ. Closed
surfaces (cube-sphere, torus, …) have no single ground plane, so they expose a
**generic** `TerrainSurfaceQuery` keyed on a world position projected onto the
surface:
...
`surfaceQuery` is `null` on flat surfaces and present for every closed surface.
Its methods take a `THREE.Vector3` world position (projected onto the surface):
...
This is the surface API used by the [torus example](/docs/examples/torus), and it
is identical regardless of the surface shape.

## Cube-sphere queries

On a cube-sphere the canonical key is a **direction from the planet center**, so
the cube-sphere exposes a `TerrainSphereQuery` that **extends**
`TerrainSurfaceQuery` with direction/lat-long variants:
...
`terrainTasks.terrainQuery` (or `terrain.runtime` in React) is the normal entry point: the task owns the CPU terrain cache, keeps it in sync with the active projection, and rebuilds the query objects when the buffer shape changes. The query objects themselves are thin wrappers created by two exported factories:

```ts
import { createTerrainQuery, createTerrainSurfaceQuery } from "@hello-terrain/three";

// Both take the `CpuTerrainCache` owned by the terrainQuery task.
const { cache } = graph.get(terrainTasks.terrainQuery);

const query: TerrainQuery = createTerrainQuery(cache);               // flat, keyed on world XZ
const surfaceQuery: TerrainSurfaceQuery = createTerrainSurfaceQuery(cache); // closed surfaces, keyed on world position
```

You only need these when implementing a custom [`SurfaceProjection`](/docs/core/projection) — its `cpu.createRuntimeQueries(cache)` hook is where the built-in projections call them — or when you want an independent query facade over the same cache. The `TerrainSphereQuery` (direction / lat-long keys) is created inside the cube-sphere projection and has no standalone factory export.
