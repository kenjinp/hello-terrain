...
During `quadtreeUpdateTask`, the graph wires `UpdateParams.tileElevationRange` from the previous-frame snapshot (scaled by `elevationScale`). Each topology's `tileBounds` uses that range to inflate LOD bounding spheres — tall terrain refines from farther away without a manual `maxHeight` padding knob.
...
`viewProjectionMatrix` is the world-to-clip matrix used for frustum culling. `residency.anchors` are world-space support spheres that keep query/readback terrain resident even when render visibility culls those tiles; see [Residency Anchors](/docs/core/residency-anchors). `tileElevationRange` returns world-space displacement min/max when data is available; `false` when the tile has no readback yet (datum-only bounds for that frame, a one-frame latency until readback data arrives).
