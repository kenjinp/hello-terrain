...
| **Torus** | `createTorusTopology` | 1 root | A closed donut surface, periodic in both axes. |
...
`createTorusTopology` produces a closed donut surface that is periodic in **both** axes: `u` sweeps around the major circle (the ring) and `v` sweeps around the tube cross-section. Because both axes wrap, there is a single root tile and every tile has a same-level neighbor. The torus is primarily a stress test of the topology + projection abstraction — adding it required no changes to the GPU pipeline, the CPU query/raycast machinery, or the React runtime.
...
The torus injects a torus `projection` (its `kind` is `"torus"`, `faceOutward: true`). `neighborSameLevel` wraps `x`/`y` modulo `2^level` on both axes, and `tileBounds` samples a 3×3 grid across each tile so that tiles spanning a full circle still produce a conservative bounding sphere.
