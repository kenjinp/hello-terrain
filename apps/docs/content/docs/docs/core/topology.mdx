...
description: Flat, infinite flat, cube-sphere, and torus topologies — what each one does, their APIs, and how to set them up in a react-three/fiber scene
---
...
All four are imported from `@hello-terrain/three` and assigned through the [`topology` param](/docs/core/params).
...
### LOD distance criteria

The bounding sphere a topology writes in `tileBounds` is what the split test consumes. With the default `quadtreeUpdate` mode (`mode: "distance"`), a tile splits when the camera is within `distanceFactor × r` of the tile's bounding-sphere center, where `r` is the surface-relative radius described above. The default `distanceFactor` is **`1.5`** — both the `quadtreeUpdate` param default and the fallback used when you pass an `UpdateParams` without one. Larger values refine from farther away (more tiles); smaller values refine later.

```ts
import { quadtreeUpdate } from "@hello-terrain/three";

graph.set(quadtreeUpdate, (prev) => ({
  cameraOrigin: prev.cameraOrigin,
  mode: "distance",
  distanceFactor: 2, // refine from farther away than the default 1.5
}));
```

`mode: "screen"` replaces the distance factor with `projectionFactor` and `targetPixels` (split when the projected pixel radius exceeds the target). See the [`quadtreeUpdate` param](/docs/core/params).
