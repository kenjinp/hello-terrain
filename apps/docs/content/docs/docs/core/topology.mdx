...
Each tile reports two radii: a conservative bounding radius `r` and an `lodRadius`. After each GPU readback, the terrain cache builds a conservative **elevation pyramid**: every leaf's min/max field values are propagated up to all ancestor tiles so coarse tiles know about mountains beneath them.

During `quadtreeUpdate`, the graph wires `UpdateParams.tileElevationRange` from the previous-frame snapshot. Each topology's `tileBounds` then displaces its sampled corners (or vertical extent on flat surfaces) by that range to grow the conservative radius `r`, which keeps the bounding sphere big enough to contain displaced geometry. Newly-seen areas fall back to the datum surface for one frame until readback data arrives — the same latency tradeoff as `elevationAtCameraXZ`.

Subdivision, however, is driven by `lodRadius` — the tile's **horizontal footprint** on the datum surface, which deliberately ignores relief. This decoupling matters: if relief fed the subdivision-size metric, a tile with a small footprint but deep elevation (a steep mountain face) would be treated as a huge tile and subdivide far past what its footprint warrants, exhausting the `maxNodes` budget. Because `lodRadius` tracks footprint, `elevationScale` controls how tall your terrain renders without distorting LOD depth. Distance-to-surface is still accounted for separately via the projection's camera-surface offset.
...
- `tileBounds` — writes camera-relative bounds. `r` is a conservative bounding radius that contains the displaced geometry (used for containment/culling); `lodRadius` is the horizontal footprint radius used as the subdivision-size metric. When an elevation range is supplied, only `r` grows with relief — `lodRadius` stays tied to the footprint so deep terrain does not over-subdivide.
