...
The `radius` and `center` you pass here are the **only** place they need to live: the GPU uniforms (`uRadius`, `uRootOrigin`) and the CPU query/raycast read them from the topology, so there is no separate `radius` param to keep in sync (the legacy `radius` param is deprecated — see [World config ownership](/docs/core/params#world-config-ownership)).
...
The same graph wiring drives a full planet — only the topology and the camera distance change. Open the **Cube-sphere** tab above and orbit around to see the six faces refine toward the camera.
...
type Topology = {
  ...
  /** World-space root tile size (flat topologies); drives `uRootSize`. */
  rootSize?: number;
  /** World-space origin / surface center; drives `uRootOrigin` and the CPU query origin. */
...
- `rootSize` / `origin` — the world config the topology was built with. The pipeline reads these (and `projection.radius`) in preference to the `rootSize` / `origin` / `radius` params, so the GPU geometry and the CPU LOD/query/raycast always agree. Flat topologies set both; curved topologies set `origin` to their `center` and leave `rootSize` undefined (their GPU projections size tiles from the radius). See [World config ownership](/docs/core/params#world-config-ownership).
...
export function Planet() {
  ...
  return (
    ...
    >
      ...
      <Terrain topology={topology} maxLevel={12} elevation={elevation}>
        {({ positionNode }) => (
...
Because the `topology` param feeds the `topologyTask`, re-assigning it re-derives the quadtree tile layout and keeps CPU LOD and GPU geometry in sync. Note there is no matching `g.set(rootSize, …)`: the topology carries its own `rootSize`/`origin`, and the uniforms read those directly. The [Infinite Flat](/docs/examples/infinite-flat) and [Cube-Sphere Planet](/docs/examples/cube-sphere) example sources show this pattern end to end.
