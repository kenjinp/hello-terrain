...
The tabs below show a minimal scene for each topology, built on the low-level graph API. Only the selected example is mounted — switching tabs unloads the previous WebGPU scene — so use the tabs rather than scrolling several live canvases at once.
