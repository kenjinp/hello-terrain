...
You do not configure this manually; it is automatic whenever the terrain readback is enabled. If you disable or heavily throttle readback via `terrainReadbackEnabled` / `terrainReadbackIntervalMs`, the pyramid is never (or rarely) refreshed and LOD falls back to the datum surface — see [Controlling readback](/docs/core/query/how-it-works#controlling-readback).
