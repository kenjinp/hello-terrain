...
During `quadtreeUpdate`, the graph wires `UpdateParams.tileElevationRange` from the previous-frame snapshot. Each topology's `tileBounds` then displaces its sampled corners (or vertical extent on flat surfaces) by that range. Newly-seen areas fall back to the datum surface for one frame until readback data arrives — the same latency tradeoff as `elevationAtCameraXZ`.
