...
| `raycast(ctx)` | The projection-specific CPU ray pick against the displaced surface; returns a plain `CpuRaycastHit` (`position` / `normal` are `{ x, y, z }`, plus `distance`), or `null` when the ray misses or the query snapshot isn't ready. |

The CPU hooks are **renderer-free**: they work on plain `{ x, y, z }` objects
(`Vec3Like`) and a plain `{ origin, direction }` ray (`RayLike`) rather than
three.js classes. A `THREE.Vector3` / `THREE.Ray` satisfies these shapes
structurally, and the runtime converts hits and samples into `THREE.Vector3`
once at the consumer boundary (`TerrainRaycast.pick`, the query objects).
...
interface CpuSurfaceOps {
  ...
  /** Unit world-space normal from a key + grid neighborhood (writes and returns `out`). */
  surfaceNormal(key: SurfaceKey, ctx: SurfaceNormalContext, out: Vec3Like): Vec3Like;
}

interface ProjectionRaycastContext {
  ray: RayLike; // { origin: Vec3Like; direction: Vec3Like }
...
Both `surfacePosition` and `surfaceNormal` write into a caller-provided scratch
object, so a sample never allocates; implementations should own any scratch they
need themselves (no module-scope state — several terrains may be alive at once).
...
3. **`cpu` hooks** — provide `createSurfaceOps()` (used by the terrain cache), `createRuntimeQueries()` (wraps the cache as `query`/`surfaceQuery`), and `raycast()`. Keep these on plain `{ x, y, z }` math (`Vec3Like` / `RayLike`) — don't import three.js in CPU code.
