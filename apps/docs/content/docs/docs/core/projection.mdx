...
interface SurfaceProjection {
  ...
  /** Representative radius; drives `uRadius` + CPU query/raycast. Undefined for flat. */
  readonly radius?: number;
  /** Surface center in world space; the topology's `origin` fallback. Undefined for flat. */
...
`radius` and `center` are authoritative: the pipeline feeds `uRadius` / `uRootOrigin` and the CPU query/raycast from the topology and its projection rather than from the legacy `radius` / `origin` params (see [World config ownership](/docs/core/params#world-config-ownership)). A custom projection that represents a curved surface should therefore set both.
