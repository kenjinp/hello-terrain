...
<Callout title="Work in progress" type="warn">
  Still noodling on this feature and page.
</Callout>

## Technical Goals
- Support a lot of nodes (max nodes probably `1 << 16` ~ 65,536)
- Support a lot of levels (meter or unit resolution for 16 meter tile size on earth radius (6,371,000m))
- Allow neighbor lookups for LOD stitching on GPU (is CPU necessary?)
- Efficient memory usage / cache locality to keep things speedy
