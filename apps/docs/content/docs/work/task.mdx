...
const ioBound = task(async (get, work, ctx) => {
  return await fetchFromNetwork(ctx.signal);
}).lane("io");
```
...
await g.run();
console.log(g.get(sum));     // 8
console.log(g.get(product)); // 15
```
...
const user = task(async (_get, work) => {
  ...
  return work(() => response.json() as User);
});
