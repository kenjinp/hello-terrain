...
It's designed to run in a hot loop (such as your render update loop), with minimal overhead. 
That's because it will bake the graph topology on the first call, and memoize (cache) unchanged dependencies. 
