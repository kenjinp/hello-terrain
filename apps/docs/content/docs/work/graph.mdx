...
<Callout type="warn">
  This graph runtime does **not** auto-register dependencies. If a task calls `get(otherTask)`,
  `otherTask` must also have been added via `g.add(otherTask)` (in any order).
</Callout>
