...
control tile resolution and quadtree depth. Per-frame inputs are split across `cameraView`, `residencyAnchors`, and `lodCriteria` params so the LOD and culling systems know what to refine.
...
Inside `useFrame`, the camera is pushed into the `cameraView` param and the
graph is run. `readCameraView` copies both the camera origin used for LOD and the
...
useFrame(async ({ camera }) => {
  g.set(cameraView, readCameraView(camera, cameraScratch));
