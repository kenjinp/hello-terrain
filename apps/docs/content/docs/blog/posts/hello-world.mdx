...
<Callout title="API updated" type="warn">
  This post describes the original **`quadtreeUpdate`** param. It has been removed: per-frame inputs are now split across **`cameraView`**, **`residencyAnchors`**, and **`lodCriteria`**. Push **`readCameraView(camera, scratch)`** into **`cameraView`** each frame — built-in hysteresis on the param replaces the manual distance checks shown below. See the [changelog](/docs/changelog) and [params](/docs/core/params) docs for the current API.
</Callout>
