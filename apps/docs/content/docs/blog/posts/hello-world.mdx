...
camera origin and optional view-projection matrix so the LOD and culling systems know what to refine.
...
Inside `useFrame`, the camera is pushed into the `quadtreeUpdate` param and the
graph is run. The helper copies both the camera origin used for LOD and the
