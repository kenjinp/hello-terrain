...
<Callout title="API updated" type="warn">
  The **`quadtreeUpdate`** param described below has been replaced by **`cameraView`**, **`residencyAnchors`**, and **`lodCriteria`**. **`quadtreeUpdateTask`** still runs when those params change, but you should set **`cameraView`** with **`readCameraView(camera, scratch)`** each frame instead of mutating `quadtreeUpdate`. Hysteresis is built into the param's equality check, so the manual **`cameraHysteresis`** pattern in this post is no longer needed. See [When does the Elevation Fn run?](/docs/core/elevation-function#when-does-the-elevation-fn-run) and the [changelog](/docs/changelog).
</Callout>
