...
The elevation function will be ran when it's upstream dependencies change, which are (broadly):
 1. the `quadtreeUpdateTask` output changes, which fires when `cameraView`, `residencyAnchors`, or `lodCriteria` change on the graph.
 2. configuration params such as `maxNodes` or `rootSize`, for example.
 3. your `elevationFn` itself changes.

Update `cameraView` each frame when the camera moves. The param uses built-in hysteresis so small movements do not dirty the graph:
...
When the camera stays still, `graph.run()` hits the fast path and compute tasks are not re-dispatched.
