...
- ~~T-junction cracks at 2:1 LOD boundaries (previously only hidden by skirts)~~ **Resolved:** a per-leaf coarse-neighbor edge mask drives true seam stitching in the vertex shader — odd boundary vertices on coarse-facing edges are snapped (position and normal) onto the chord between their even edge-neighbors, for both flat and cube-sphere topologies. Skirts remain as a safety net.
