...
import { cubeSphereRaycast, type SphereRaycastParams } from '../query/cpu-raycast';
...
export function createCubeSphereProjection(config: CubeSphereProjectionConfig): SurfaceProjection {
    ...
    const surfaceOps: CpuSurfaceOps = {
        ...
        surfaceNormal(key: SurfaceKey, ctx: SurfaceNormalContext): Vector3 {
            ...
            const hLeft =
                sampleGridBilinear(ctx.elevation, ctx.shape, ctx.leafIndex, ctx.gx - 1, ctx.gy) *
                scale;
            const hRight =
                sampleGridBilinear(ctx.elevation, ctx.shape, ctx.leafIndex, ctx.gx + 1, ctx.gy) *
                scale;
            const hUp =
                sampleGridBilinear(ctx.elevation, ctx.shape, ctx.leafIndex, ctx.gx, ctx.gy - 1) *
                scale;
            const hDown =
                sampleGridBilinear(ctx.elevation, ctx.shape, ctx.leafIndex, ctx.gx, ctx.gy + 1) *
                scale;
