...
export interface TileBoundsContext {
    ...
    /** Releases the GPU buffer and reduction kernel backing this context. */
    dispose?: () => void;
}
...
export const tileBoundsContextTask = task<{ renderer: WebGPURenderer }>((get, work, ctx) => {
    const elevationFieldContext = get(createElevationFieldContextTask);
    const dirtyVisibleSlotStorage = get(dirtyVisibleSlotStorageTask);
    const maxNodesValue = get(maxNodes);
    const edgeVertexCount = get(innerTileSegments) + 3;

    type Context = TileBoundsContext & { kernel: ReturnType<typeof buildReductionKernel> };
    return work((prev?: Context): Context => {
        prev?.dispose?.();
        ...
        const dispose = () => {
            ...
            disposeStorageBufferState(renderer, { attribute, node });
        };
        return { data, attribute, node, kernel, dispose };
    });
})
