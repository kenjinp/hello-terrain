...
describe('quadtree/topology/torus', () => {
    ...
    it('keys radii and center changes for cache invalidation', () => {
        ...
        expect(
            new Set([base.cacheKey, wider.cacheKey, thicker.cacheKey, shifted.cacheKey]).size
        ).toBe(4);
    ...
    it('wraps neighbors around both periodic axes', () => {
        ...
        expect(topology.neighborSameLevel({ space: 0, level: 2, x: 1, y: 1 }, Dir.RIGHT, out)).toBe(
            true
        );
    ...
    it('expands bounds when an elevation range is provided', () => {
        ...
        topology.tileBounds({ space: 0, level: 2, x: 1, y: 0 }, camera, displaced, {
            min: 0,
            max: 80,
        });
