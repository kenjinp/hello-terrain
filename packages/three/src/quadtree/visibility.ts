...
export function computeTileVisibility(
    ...
    for (let i = 0; i < candidateCount; i += 1) {
        ...
        // When an elevation-range provider is wired but has no data for this tile
        // yet (fresh tile, readback pending), its datum-height bounds can sit far
        // outside the frustum (e.g. camera hundreds of meters above the datum).
        // Culling it would deadlock: culled tiles are never computed, so their
        // bounds never arrive, so they stay culled. Treat such tiles as visible
        // until their first real bounds land. Without a provider, the base
        // geometry bounds are authoritative and culling applies as usual.
        if (hasElevationRange || options.elevationRangeForTile === undefined) {
            if (
                canUseFrustum &&
