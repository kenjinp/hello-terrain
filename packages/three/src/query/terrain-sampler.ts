...
import { createTileIndexFromDirection, createTileIndexFromWorldPosition } from './gpuSpatialIndex';
 ...
 * the location).
 */
function packedSampleFromTileResult(params: CreateTerrainSamplerParams, tileResult: Node): Node {
    const tileIndex = int(tileResult.x).toVar();
    const safeTileIndex = tileIndex.max(int(0)).toVar();
    const fieldU = tileLocalToFieldUV(tileResult.y, params.uniforms.uInnerTileSegments).toVar();
    const fieldV = tileLocalToFieldUV(tileResult.z, params.uniforms.uInnerTileSegments).toVar();
...
export function augmentCubeSphereSampler(
    ...
    sampler.sampleElevationByDirection = Fn(([direction]: [Node]) => terrainSampleByDir(direction).x);
...
export function createTerrainSampler(params: CreateTerrainSamplerParams): TerrainSampler {
    ...
    const sampleTerrain = Fn(([worldX, worldZ]: [Node, Node]) => terrainSampleAt(worldX, worldZ));
    const sampleElevation = Fn(([worldX, worldZ]: [Node, Node]) => terrainSampleAt(worldX, worldZ).x);
