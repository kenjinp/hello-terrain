...
import { cubeFaceBasis, cubeFaceFromDirection, cubeFaceUVFromDirection } from '../tsl/cubeSphere';
...
export function createGpuSpatialIndex(
    maxEntries: number,
    ...
    const context: GpuSpatialIndexContext = {
        data,
        size,
        mask: size - 1,
        stampGen,
    ...
    context.dispose = () => disposeStorageBufferState(renderer, context);
    return context;
}
...
export function uploadGpuSpatialIndex(
    ...
    if (gpuIndex.size !== cpuIndex.size) {
        throw new Error(`Spatial index size mismatch (gpu=${gpuIndex.size}, cpu=${cpuIndex.size}).`);
    }
...
const createGpuSpatialLookup = (spatialIndex: GpuSpatialIndexContext) => {
    ...
    return Fn(([space, level, x, y]: [Node, Node, Node, Node]) => {
        ...
        Loop(slotCount, () => {
            ...
            If(ks.equal(s).and(kl.equal(l)).and(kx.equal(xx)).and(ky.equal(yy)), () => {
