...
export interface TerrainFieldStorage {
    ...
    /** Releases the GPU texture backing this storage. */
    dispose(): void;
}
...
export function ArrayTextureBackend(
    ...
    const tex = new StorageArrayTexture(edgeVertexCount, edgeVertexCount, tileCount);
...
export function storeTerrainField(
    ...
    if (storage.backendType === 'array-texture' || storage.backendType === 'texture-3d') {
        return textureStore(storage.texture, uvec3(int(ix), int(iy), int(tileIndex)), value);
    }
