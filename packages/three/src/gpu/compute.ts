...
export function compileComputePipeline(
    ...
    /** Disposes all cached compute kernels so renderer pipelines are released. */
    dispose: () => void;
} {
    ...
    function buildLinearStagedKernels(workgroupSize: [number, number, number]) {
        return stages.map((stage, stageIndex) =>
            Fn(() => {
                ...
                const inBounds = uint(linearIndex).lessThan(uTotalVertexCount).toVar();

                If(inBounds, () => {
                    const globalIndex = int(nodeIndex).mul(verticesPerNode).add(localIndex);
                    stage(nodeIndex, globalIndex, localUVCoords, localCoordinates, texelSize);
                });
            })()
                .computeKernel(workgroupSize)
                .setName(
                    label ? `${label}.linearStage${stageIndex}` : `compute.linearStage${stageIndex}`
                )
        );
    }
    ...
    function execute(renderer: WebGPURenderer, instanceCount: number) {
        ...
        if (dispatchMode === 'linear') {
            ...
            for (let stageIndex = 0; stageIndex < stagedKernels.length; stageIndex += 1) {
                renderer.compute(stagedKernels[stageIndex], [dispatchX, 1, 1]);
                if (
                    midPipelineExecute &&
    ...
    function dispose() {
        const disposeKernel = (kernel: CompiledKernel) => {
            (kernel as { dispose?: () => void })?.dispose?.();
        };
        if (singleKernel) {
            disposeKernel(singleKernel);
            singleKernel = undefined;
        }
        for (const kernels of stagedKernelCache.values()) kernels.forEach(disposeKernel);
        stagedKernelCache.clear();
        for (const kernels of linearStagedKernelCache.values()) kernels.forEach(disposeKernel);
        linearStagedKernelCache.clear();
    }
