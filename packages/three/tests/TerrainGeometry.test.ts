...
function posAt(g: TerrainGeometry, edgeVertexCountWithSkirt: number, ix: number, iy: number) {
    const pos = g.getAttribute('position');
    const i = idx(edgeVertexCountWithSkirt, ix, iy);
    return [pos.getX(i), pos.getY(i), pos.getZ(i)] as const;
}

function normAt(g: TerrainGeometry, edgeVertexCountWithSkirt: number, ix: number, iy: number) {
    const n = g.getAttribute('normal');
    const i = idx(edgeVertexCountWithSkirt, ix, iy);
    return [n.getX(i), n.getY(i), n.getZ(i)] as const;
}

function uvAt(g: TerrainGeometry, edgeVertexCountWithSkirt: number, ix: number, iy: number) {
...
describe('TerrainGeometry', () => {
    it('throws on invalid innerSegments', () => {
        ...
        expect(() => new TerrainGeometry(Number.NaN)).toThrow(/Invalid innerSegments/);
        expect(() => new TerrainGeometry(Number.POSITIVE_INFINITY)).toThrow(/Invalid innerSegments/);
    ...
    it('UVs differ for extendUV=true vs extendUV=false as expected', () => {
        ...
        expect(uvAt(gExtended, edgeVertexCountWithSkirt, last, last)).toEqual([1, 0]);
