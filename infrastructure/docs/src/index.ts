...
const uploadedAssets = uploadAssetsToS3(
  buckets.mainBucket,
  "../../apps/docs/out",
  "docs"
);
...
// Invalidate CloudFront cache when assets change
// Uses content hash as trigger - invalidation runs when any asset content changes
const contentHash = pulumi
  .all(uploadedAssets.map((obj) => obj.etag))
  .apply((etags) => etags.join(","));

new command.local.Command(
  "cache-invalidation",
  {
    create: pulumi.interpolate`aws cloudfront create-invalidation --distribution-id ${cdn.distribution.id} --paths "/*"`,
    triggers: [contentHash],
  },
  { dependsOn: uploadedAssets }
);
