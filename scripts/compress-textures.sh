...
TEXTURES_DIR="$PROJECT_ROOT/apps/docs/public/assets/terrain-textures"
BACKUP_DIR="$TEXTURES_DIR/.backup"
...
if [ ! -d "$TEXTURES_DIR" ]; then
    echo -e "${RED}Error: Textures directory not found: $TEXTURES_DIR${NC}"
    exit 1
fi

echo -e "Textures directory: ${BLUE}$TEXTURES_DIR${NC}"
...
for texture_dir in "$TEXTURES_DIR"/*/; do
...
if [ "$CREATE_BACKUP" = true ]; then
    ...
    echo -e "To restore: cp -r $BACKUP_DIR/* $TEXTURES_DIR/"
fi
